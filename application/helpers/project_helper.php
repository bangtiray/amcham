...
function widget_sidebar(){
	...
	//========== event  ===========
		...
		$CI->db->limit(2);
		$CI->db->order_by('a.start_date','desc');
...
function proses_member_backend($idedit = "") {
	...
	#sector
		$id_parent_sector = $CI->sector_model->get_idotherparent();
		if (in_array($id_parent_sector, $sector)) {
			if ($other_sector_name) { // name other sector
				...
				$checksector                        = $CI->sector_model->findBy(array('uri_path'=>generate_url($other_sector_name)),1);

				if ($checksector) {
					$insert_sector_id          = $CI->sector_model->update_frontend($insert_sector,$checksector['id']);
				}else{
					$insert_sector_id          = $CI->sector_model->insert_frontend($insert_sector);
				}
