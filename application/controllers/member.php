...
class Member extends CI_Controller {
	...
	function privileges(){
		...
		$data['list_category_privileges'] = $this->membership_privileges_category_model->findBy();
	...
	function list_privileges(){
		...
		if (empty(array_filter($check))) {			
			...
			foreach ($check as $key => $value) {
				...
				$ret .=	      '<div class="thumbnail-amcham thumb-product">' .getImg($value['img'],'small').'</div>';
				$ret .=	    '</div>';
				...
				$ret .=	        '</div>';
				$ret .=	        '<div class="media-body">'.$value['privileges_website'].'</div>';
				$ret .=	      '</div>';
				$ret .=	    '</div>';
				$ret .=	  '</div>';
		...
		if($user_sess_data){
			...
			// if ($data_invoice['bank_account'] != "" or !is_null($data_invoice['bank_account'])) {
				...
				$check_file = $this->paymentconfirmationfiles_model->findBy(
								array('type_id'=>2,'payment_confirmation_id'=>$data_invoice['id']),1
							);
				if ($check_file) {
	...
	function register($category){		
		$user_sess_data = $this->session->userdata('MEM_SESS');
		if($user_sess_data){ redirect('/'); }
		
		$this->load->model('bank_account_model');
		$data_bank = $this->bank_account_model->findBy();
		foreach ($data_bank as $key => $value) {
			$data['list_bank'][] = array(
				'list_bank_id' => $value['id'],
				'list_bank_name' => $value['bank_name']
			);
		}
		...
		switch ($category) {
			case 'company': 
				...
				$data['is_required']          = '';
				$data['is_hide']              = '';
				$data['form_applicant_name']  = 'COMPANY DETAILS';
				$data['form_company_address'] = 'Company Address';
			break;
			...
			case 'individual': 
				...
				$data['is_hide']              = '';
				$data['form_applicant_name']  = 'APPLICANT DETAILS';
				$data['form_company_address'] = 'Address';
			break;
	...
	function register_proses($type_input){
		$post                       = $this->input->post();
    	...
    	//bank transfer
		if ( $post['invoice_number'] != "" ) {
			// update invoice 
			$data_invoice                   = $this->paymentconfirmation_model->findBy(array('invoice_number'=>$post['invoice_number']),1);
		 	$update_data_invoice['member_id']    = $id_member;  
	        $update_data_invoice['is_paid'] = $post['success_invoice'] == "true"? 1 : 0 ;  
	        $update_data_invoice['id_ref_payment_type'] = 2 ;  
	        $this->paymentconfirmation_model->update_frontend($update_data_invoice,$data_invoice['id']);
		 }else{
		 	// insert invoice
			$invoice_number = 'M-'.rand(1000000000000,9999999999999);

	        $data_invoice['id_ref_payment_category'] = 2; 
	        $data_invoice['invoice_number']          = $invoice_number ; 
	        $data_invoice['member_id']               = $id_member ; 
	        $data_invoice['is_paid']                 = 0 ;  
	        $data_invoice['id_ref_payment_type'] = 2 ;  
		...
		if ($post['payment_method'] != 'payment_online') {
			...
			$email_user['invoice_number']    = $invoice_number; 
			$email_user['name_price']        = $price_name; 
			$email_user['price']             = number_format($price_amount); 
			$email_user['grand_total']       = number_format($price_amount);
			$email_user['bank_page_content'] = $bank_page_content; 
			$email_user['link']              = base_url_lang().'/payment_confirmation/'.$invoice_number;
