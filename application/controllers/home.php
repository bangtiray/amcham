...
/* function update_gallery_tags()
    {
      ...
      // Other Special Events 177
      // Initiative Indonesia 178
...
/*    function migrasi_check_user(){
  ...
  /*  function migrasi_kita_company_2(){
   ...
   /* function migrasi_data(){
    ...
    // function migrasi_member()
    // {
    //   $CI =& get_instance();
    //   $CI->migrasi   = $CI->load->database('migrasi', TRUE);

    //   // $data_user = $this->db->get('jos_users')->result_array();
       
    //   // $CI->migrasi->where('block = 1');
    //   $CI->migrasi->where('id > 2032');
    //   $data_user = $CI->migrasi->get('jos_users')->result_array();
    //   // id  migrasi tapi sudah di hapus
    //   // 232,233,235,236,237,239,240,241,244,247,248,249,251,252,254,255,257,264,265,271,272,276,277,278,279,280,281,283,284,285,288,293,297,298,299,302,303,305,307,308,309,310,312,316,318,320,321,323,325,329,333,335,336,338,345,348,352,361,362,364,371,375,376,377,378,381,382,384,385,386,388,392,394,400,401,404,406,407,408,416,420,421,423,424,429,431,432,433,434,436,440,443,444,448,451,452,455,456,457,458,460,461,464,469,470,474,475,476,478,481,482,486,489,491,492,494,495,496,498,500,501,503,504,505,507,510,511,514,517,518,519,521,522,523,525,526,527,528,529,532,533,537,538,541,547,549,550,552,554,555,556,557,559,560,561,562,563,564,566,567,568,569,574,582,583,584,585,589,590,591,592,593,595,599,600,601,602,605,606,610,611,612,613,622,625,627,630,632,634,637,640,642,644,653,657,659,660,661,662,663,666,668,670,671,673,675,676,680,681,686,689,693,699,700,701,702,705,706,708,709,710,712,713,717,721,723,724,726,727,731,732,737,739,740,741,743,745,746,747,749,750,751,753,756,760,762,763,764,765,771,781,801,811,851,871,891,901,911,921,941,981,991,1021,1051,1121,1131,1141,1171,1191,1211,1241,1351,1361,1461,1471,1481,1501,1511,1521,1531,1541,1546,1553,1554,1555,1558,1560,1566,1570,1571,1574,1575,1577,1578,1582,1583,1585,1586,1588,1589,1591,1593,1594,1596,1598,1599,1600,1601,1602,1605,1608,1610,1612,1613,1614,1619,1622,1626,1627,1629,1630,1631,1633,1637,1639,1641,1642,1643,1644,1645,1646,1647,1648,1649,1650,1651,1654,1655,1659,1660,1663,1664,1665,1666,1670,1674,1675,1676,1677,1678,1681,1682,1688,1689,1691,1692,1698,1699,1700,1701,1705,1706,1707,1708,1709,1710,1711,1713,1714,1715,1716,1720,1722,1723,1724,1726,1727,1728,1729,1730,1731,1736,1737,1738,1740,1742,1744,1748,1750,1751,1754,1758,1760,1763,1772,1774,1775,1776,1778,1779,1780,1781,1782,1783,1784,1785,1787,1790,1791,1793,1794,1798,1799,1800,1802,1803,1804,1808,1814,1815,1816,1818,1822,1823,1825,1827,1831,1834,1836,1838,1841,1843,1847,1851,1852,1853,1857,1860,1861,1865,1866,1867,1868,1870,1871,1876,1879,1880,1882,1892,1894,1897,1899,1900,1901,1902,1903,1904,1913,1921,1929,1956,1964,1969
      
    //   foreach ($data_user as $key => $user) {
    //     $data_user_profile = $CI->migrasi->where('id'=>$user['id'])->get('jos_user_profiles')->result_array();
    //     $user_name         = explode(', ', $user_name['name']);
    //     $data_insert[$key]     = [
    //       'firstname' => $user_name[0],
    //       'lastname' =>$user_name[1] ?$user_name[1] :"",
    //       'create_date' => $user['registerDate'],
    ...
    //    // // http://amcham.or.id/images/joomd/
    //    // jos_joomd_type1
    //    // field_32
    //    // field_33
    //    // field_34
    ...
    //     foreach ($data_user_profile as $profile) {
    //       switch ($profile) {

    //         case $profile['profile.address1']:
    //           $data_insert[$key]['address']  .= $profile['profile.address1']; break;
    //         case $profile['profile.address2']:
    //           $data_insert[$key]['address'] .= $profile['profile.address2']; break;
    //         case $profile['profile.company_name']:
    //           $data_insert[$key]['company_name'] = $profile['profile.company_name']; break;
    //         case $profile['profile.email1']:
    //           $data_insert[$key]['email1'] = $profile['profile.email1']; break;
    //         case $profile['profile.job_position']:
    //           $data_insert[$key]['job'] = $profile['profile.job_position']; break;
    //         case $profile['profile.phone1']:
    //           $data_insert[$key]['m_t_phone'] = $profile['profile.phone1']; break;
    //         case $profile['profile.phone2']:
    //           $data_insert[$key]['m_m_phone'] = $profile['profile.phone2']; break;
    //         case $profile['profile.website']: 
    //           $data_insert[$key]['website']   = $profile['profile.website']; break;
    //       }
    //     }
    //   }
    ...
    function index(){
        ...
        foreach ($data_news as $key => $value) {
            ...
            $temp['img_url']     = base_url().'images/article/large/'.$value['img'];
