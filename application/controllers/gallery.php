...
class Gallery extends CI_Controller {
    ...
    function photo(){
       ...
       $this->db->select(' a.is_delete as tags_is_del,
                            b.name,
                            b.teaser,
                            b.description,
                            b.is_delete as gallery_is_del,
                            b.id_gallery_category,
                            b.id_status_publish,
                            b.img,
                            b.publish_date,
                            b.uri_path,
        ...
        $this->db->join('gallery b', "b.id = a.id_gallery");
        $this->db->join('tags c', "c.id = a.id_tags");
        $this->db->where('b.id_status_publish','2');
        $this->db->where('b.is_delete','0');
        $this->db->where('b.publish_date <' ." '".date('Y-m-d')."'");

        $datalist =  $this->db->get_where('gallery_tags a',array('b.id_lang'=>$id_lang))->result_array();
    ...
    function morephoto($type,$page,$ret=0){
        ...
        $data   = $this->db->get_where('gallery_tags a',array('b.id_lang'=>$id_lang))->result_array();
    ...
    function detailphoto($uri_path){
        ...
        if(!$data){ 
            $data = $this->gallerymodel->fetchRow(array('a.uri_path'=>$uri_path));
        }
