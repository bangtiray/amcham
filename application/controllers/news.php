...
class News extends CI_Controller {
    ...
    function index(){
        ...
        if($menu && $menu['id_parent']==0){ // kalo menu sebagai parent category
            ...
            if ($uri_path == 'amcham-reports' || $uri_path =='other-reports') { // custom uri path
                $view = 'report';
        ...
        foreach ($news as $key => $value) {
            ...
            $news[$key]['news_teaser']   = ($uri_path == 'other-reports' || $uri_path == 'amcham-reports' || $uri_path == 'report-and-publications') ?$value['teaser']:$news[$key]['news_teaser']; //kalo public
            $news[$key]['idx']            = ($value['filename']) ? md5plus($value['id']) : '';
            $news[$key]['invis_download'] = ($value['filename']) ? '' : 'hide';
            $news[$key]['button']         = ($value['filename']) ? 
            '<a class="tangan btn-red link-reports '. $news[$key]['invis_download'].'" id="'.$news[$key]['idx'].'" d-title="news" newtab="true" preview="true" onclick="download_file(this)">  Read the report</a>' :
             '<a href="'.$news[$key]['news_url'].'" class="btn-red link-reports">See More</a>';
        ...
        $data['dsp_load_more'] = $this->more($uri_path,$data['paging'],1,$keyword,$m,$y) ? '' : 'hide';
        ...
        if ($post) {
            ...
            return json_encode($data_json);exit;
        }else{
    ...
    function detail(){
        ...
        $data['page_name'] = in_array( $get_uri_cat['id'], id_child_news('56',1) )? 'US Indonesia Investment Initiative': $get_uri_cat['name'];
    ...
    function get_download(){
        ...
        $file = db_get_one('news','filename',array(md5field('id')=>$post['idx']));
        if ($file) {
            $file = preg_replace('/\s+/', '_', $file);
            // $url_file = base_url().'/file_upload/'.$file;
            $base_url   = base_url();
            $data['path'] =  $base_url.'viewer/web/viewer.html?file='.$base_url.'file_upload/'.$file;
        } else {
            $data['path'] = 'error';
        }
    ...
    function more($uri_path,$page,$ret=0,$keyword,$month,$year){
        ...
        foreach ($data as $key => $value) { 
            ...
            // $news_teaser   = ($uri_path == 'other-reports' || $uri_path == 'amcham-reports' || $uri_path == 'report-and-publications') ?$value['teaser']:$news[$key]['news_teaser']; //kalo public
            $idx            = ($value['filename']) ? md5plus($value['id']) : '';
            $invis_download = ($value['filename']) ? '' : 'hide';
            $button = ($value['filename']) ? 
            '<a class="tangan btn-red link-reports '.$invis_download.'" id="{idx}" d-title="news" newtab="true" preview="true" onclick="download_file(this)">  Read the report</a>' : '<a href="'.site_url('news/detail/'.$news_url).'" class="btn-red link-reports">See More</a>';

            $news_writer   = $value['writer'];
                if($uri_path == 'other-reports' || $uri_path == 'amcham-reports' || $uri_path == 'report-and-publications'){
                    ?>
                        ...
                        <div class="media media-latest">
                            ...
                            <div class="media-body">
                                <div class="widget-white">
                                    <div class="event-type "><?=$news_category?>  <!-- |  <?=$news_date?> --></div> <!-- Sep 29, 2017 -->
                                    <?=$button?>
