...
class Event extends CI_Controller {
	...
	function proses($idedit=''){
		...
		foreach ($post['teaser'] as $key => $value){
			...
			/* end check record */
			foreach ($post['detail_img'] as $key2 => $value) {
				$temp['filename']     = $post['detail_img'][$key2];
				$temp['description']  = $post['description_images_detail'][$key2];
				$temp['is_share']     = $post['is_share'][$key2];
				$temp['is_delete']    = 0;
